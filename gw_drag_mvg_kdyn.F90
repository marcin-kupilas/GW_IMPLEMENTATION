module gw_drag
    ...
    subroutine gw_init()
         ...
         if (use_gw_chem) then
          !add values for each peaks 
            do i = 1, 6
               ...
               call addfld('k_tot_orog'//cn//'RDGBETA' ,(/ 'lev' /), 'A','m2/s', &
            ...
            call addfld ('EKGW_orog_tot_BETA' ,(/ 'ilev' /), 'A','M2/S', &
               'Effective Kzz orography')
            call addfld ('k_tot_orog_tot_BETA', (/ 'lev' /), 'A','m2/s', &
      ...
      if (use_gw_front) then
         ...
         !MVG
         !define outputs as a function of GW sources for gw_chem terms (N.B. this is a copy of gw_spec_addflds 
      ...
      if (use_gw_convect_dp) then
         ...
         call addfld ('HDEPTH',horiz_only,    'A','km', &
              'Heating Depth')
         !MVG
         !define outputs as a function of GW sources for gw_chem terms
         if (use_gw_chem) then
     ...
     if (use_gw_chem) then 			!MVG
         ...
         call addfld ('xi_tot',(/ 'lev' /)  ,  'A',' ', &
              'Instability parameter (Xi_tot) (over entire spectrum)')
         call addfld ('k_e_tot', (/ 'lev' /), 'A','m2/s', &
              'Wave energy flux (over entire spectrum)')
         call addfld ('k_tot_tot',(/ 'ilev' /), 'A','m2/s', &
              'Total diffusivity (1+xi*kzz+k_wave) (over entire spectrum)')
         call addfld ('k_dyn_unbounded_tot', (/ 'ilev' /), 'A','m2/s', &
              ' ')
    
         call addfld ('var_gwt_tot', (/ 'lev' /), 'A','K2', &
              'Variance of gw temperature perturbation')
         call addfld ('dtdz', (/ 'lev' /), 'A','K/m', &
              'Dt/Dz environment')
         call addfld ('brnt_v', (/ 'lev' /), 'A','s-1', &
    ...
    subroutine gw_tend(state, pbuf, dt, ptend, cam_in, flx_heat)
      ...
      real(r8) :: k_tot_tot(state%ncol,pver+1) 
      ...
      real(r8) :: var_gwt(state%ncol,pver)   ! Variance of T'
      real(r8) :: k_tot(state%ncol,pver+1)
     
      ! Dt/Dz environment 
      real(r8) :: dtdz(state%ncol, pver)
      ! Interface Brunt-Vaisala frequency computed locally in gw_chem.
      real(r8) :: brnt_v(state%ncol,pver)
    
      !unbounded value of K_dyn and K_dyn_unbounded/K_zz_max ratio 
      real(r8) :: k_dyn_unbounded(state%ncol,pver+1)
      real(r8) :: k_dyn_unbounded_tot(state%ncol,pver+1)
      real(r8) :: times_kzz_max(state%ncol,pver+1) 
      ...
      if (use_gw_convect_dp) then
        ...
        IF (use_gw_chem) then  							!MVG
         ...
         call gw_drag_prof(ncol, band_mid, p, src_level, tend_level,  dt, &
            ...
            call effective_gw_diffusivity(ncol, band_mid, wavelength_mid, p, dt,     &
                 ...
                 vramp, k_wave, xi, k_e, zm, zi, var_gwt, dtdz, brnt_v, k_tot,       &
             dttdf, ttgw, qtgw, state1%lat(:ncol), state1%lon(:ncol), 		 &
             k_dyn_unbounded, times_kzz_max)
        ...
        do k = 1, pver+1 
         k_tot_tot(:,k) = k_tot_tot(:,k) + k_tot(:,k)	
         k_dyn_unbounded_tot(:,k)= k_dyn_unbounded_tot(:,k) + k_dyn_unbounded(:,k)
        enddo
    
            !write in history file
        call gw_chem_outflds(beres_dp_pf, lchnk, ncol, k_wave, &
                                xi, k_e, k_tot, egwdffi, times_kzz_max)
      ...
      if (use_gw_rdg_beta) then
         ...
         IF (use_gw_chem) then                              !MVG
            ...
            rdg_beta_cd_llb, trpd_leewv_rdg_beta,     &
            ptend, flx_heat, k_wave, xi, k_e, k_tot, egwdffi, &
            var_gwt, state1%lat(:ncol), state1%lon(:ncol),  		 &
        k_dyn_unbounded, use_gw_chem=use_gw_chem)
      ...
      if (use_gw_chem) then 			!MVG
         ...
         call outfld ('k_tot_tot', k_tot_tot, ncol, lchnk)
         call outfld ('k_dyn_unbounded_tot', k_dyn_unbounded_tot, ncol, lchnk)
        
         call outfld ('var_gwt_tot', var_gwt_tot, ncol, lchnk)
         call outfld ('dtdz', dtdz, ncol, lchnk)     
         call outfld ('brnt_v', brnt_v, ncol, lchnk)
    ...
    subroutine gw_rdg_calc( &
       ...
       k_e_orog_tot, k_tot_orog_tot, egwdffi_orog_tot, &
       var_gwt_orog_tot, lat, lon, k_dyn_unbounded_orog_tot, use_gw_chem) !MVG optional arguments for gw_chem
      ...
      real(r8) :: times_kzz_max(ncol,pver+1)
       ...
       xi_orog_tot=0._r8
       k_e_orog_tot=0._r8
       k_tot_orog_tot=0._r8
       egwdffi_orog_tot=0._r8
       var_gwt_orog_tot=0._r8
       k_dyn_unbounded_orog_tot=0._r8
       ...
       do nn = 1, n_rdg
        ...
        ! Solve for the drag profile 
            ...
            call effective_gw_diffusivity(ncol, band_oro, wavelength_mid, p, dt,     &
                 t, rhoi, nm, ni, c, tau, egwdffi, ubi, q, dse, dttke, tend_level,   &
                 vramp, k_wave_orog, xi_orog, k_e_orog, zm, zi, var_gwt_orog, dtdz,  &
             brnt_v, k_tot_orog, dttdf, ttgw, qtgw, lat, lon, k_dyn_unbounded_orog, &
             times_kzz_max, kwvrdg=kwvrdg)
          ...
          do k = 1, pver !add up contributions from all ridges
           ...
           var_gwt_orog_tot(:,k) = var_gwt_orog_tot(:,k) +  var_gwt_orog(:,k)
            ...
            !  add up diffusion coefficients  from all ridges
             do k = 1, pver+1
              k_tot_orog_tot(:,k) = k_tot_orog_tot(:,k) + k_tot_orog(:,k)
              k_dyn_unbounded_orog_tot(:,k)=k_dyn_unbounded_orog_tot(:,k)+k_dyn_unbounded_orog(:,k)
         ...
         call outfld('k_tot_orog'//cn//'RDG'//trim(type),  k_tot_orog,    ncol, lchnk) 
       ...
       if (use_gw_chem) then !MVG
         ...
         call outfld ('k_tot_orog_tot_'//trim(type), k_tot_orog_tot, ncol, lchnk)
    ...
    subroutine gw_spec_addflds(prefix, scheme, band, history_defaults)
      ...
      do l=-band%ngwv,band%ngwv
        ...
        ! add wind tendencies associated to each wave (u' tendencies spectrum) and total tau  !! MVG !!
        var_name_at_c_u   = var_fld_name(l, prefix, u_tend=.true.)
        var_name_at_c_tau = var_fld_name(l, prefix, u_tend=.false.)
        var_descr_u=trim(scheme)//" gwut at c= "//trim(fnum)//" m/s"
    ...
    character(len=10) pure function var_fld_name(l, prefix, u_tend)
      ...
      character(len=1), intent(in) :: prefix
      ! u_tend or tau?
      logical, intent(in) :: u_tend
    
      character(len=2) :: num_str
    
      var_fld_name = trim(prefix)
    
    
      if (u_tend) then
         var_fld_name = trim(var_fld_name)//"GWUT"
      else
         var_fld_name = trim(var_fld_name)//"TAU"
      end if
    
      if (l < 0) then
         var_fld_name = trim(var_fld_name)//"n"
      else
         var_fld_name = trim(var_fld_name)//"p"
      end if
    ...
    subroutine gw_chem_addflds(prefix, scheme, band, history_defaults)
      ...
      call addfld (trim(prefix)//'_k_e',(/ 'lev' /), 'A','m2/s', &
           trim(scheme)//' k_e')
      call addfld (trim(prefix)//'_k_tot',(/ 'ilev' /), 'A','m2/s', &
           trim(scheme)//' k_tot')
      call addfld (trim(prefix)//'_times_kzz_max',(/ 'ilev' /), 'A',' ', &
           trim(scheme)//' times_kzz_max')
    ...
    subroutine gw_chem_outflds(prefix, lchnk, ncol,  k_wave, &
      ...
      real(r8), intent(in) :: k_tot(ncol,pver)
    
      real(r8), intent(in) :: egwdffi(ncol,pver+1)
      real(r8), intent(in) :: times_kzz_max(ncol,pver+1)
      ...
      call outfld(trim(prefix)//'_k_tot', k_tot, ncol, lchnk)
      call outfld(trim(prefix)//'_times_kzz_max', times_kzz_max, ncol, lchnk)
