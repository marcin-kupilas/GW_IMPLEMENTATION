...
!WUHU FENG, JOHN PLANE, 18/09/2018
      ...
      do i=1,ncol
!-----------------------------------------------------------------
!ADDED BY WUHU FENG AND John Plane
!-----------------------------------------------------------------
!!-----------------------------------------------------------------
      logt=log10(temp(i,k))
...
!-----------------------------------------------------------------
!      if (temp(i,k) < 147._r8) then
!     rxt(i,k,fe40_ndx)=4.86_r8*(temp(i,k))**0.5_r8*pmc_vsa(i,k)
!     else
      rxt(i,k,fe40_ndx) = 0.0_r8
!     endif
! NO PMC experiment
!     rxt(:,k,fe40_ndx) = 0.0_r8
!-----------------------------------------------------------------
!       ... Fe+  -> Fe_PMC: gamma=1.,
!-----------------------------------------------------------------

!     rxt(:,k,fe41_ndx) = 4.86_r8 *(temp(:ncol,k))**0.5_r8*pmc_vsa(:ncol,k)
      rxt(i,k,fe41_ndx) = rxt(i,k,fe40_ndx)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!       ... FeOH  -> Fe_PMC: gamma=1.,
!-----------------------------------------------------------------

!     rxt(:,k,fe42_ndx) = 4.26_r8 *(temp(:ncol,k))**0.5_r8*pmc_vsa(:ncol,k)
      rxt(i,k,fe42_ndx) = rxt(i,k,fe40_ndx)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!       ... FeO2H2  -> Fe_PMC: gamma=1.,
!-----------------------------------------------------------------

!     rxt(:,k,fe43_ndx) = 3.83_r8 *(temp(:ncol,k))**0.5_r8*pmc_vsa(:ncol,k)
      rxt(i,k,fe43_ndx) = rxt(i,k,fe40_ndx)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!       ... FeO3  -> Fe_PMC: gamma=1.,
!-----------------------------------------------------------------

!     rxt(:,k,fe44_ndx) = 3.57_r8 *(temp(:ncol,k))**0.5_r8*pmc_vsa(:ncol,k)
      rxt(i,k,fe44_ndx) = rxt(i,k,fe40_ndx)
!-----------------------------------------------------------------
!-----------------------------------------------------------------
!       ... FePMC  -> FeOH: Evaporation of PMC to release FeOH only
!-----------------------------------------------------------------
!       if(temp(i,k) >=155._r8)then
!      rxt(i,k,fe45_ndx) = 1.e-4_r8
!       else
       rxt(i,k,fe45_ndx) = 0._r8
!      endif
!-----------------------------------------------------------------
        enddo
        enddo
