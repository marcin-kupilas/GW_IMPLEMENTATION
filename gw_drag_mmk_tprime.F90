...
! Modified by: Maria Vittoria Guarino (MVG)
!
!--------------------------------------------------------------------------
!
! Further incremental improvements from MVG 
! implemented by Marcin Mikolaj Kupilas (MMK)
!
!--------------------------------------------------------------------------
...
subroutine gw_init()
 ...
 if (use_gw_chem) then 			!MVG
     ...
     call addfld ('gw_t_prime_tot', (/ 'lev' /), 'A','K2', &
          'Total temperature perturbation')
     ! MMK 
...
subroutine gw_tend(state, pbuf, dt, ptend, cam_in, flx_heat)
  ...
  real(r8) :: gw_t_prime_tot(state%ncol,pver) ! Total gw temperature perturbation 
  ...
  ! MMK new 
  real(r8) :: gw_t_prime(state%ncol,pver) ! Total temperature perturbations for each source
    ...
    IF (use_gw_chem) then  							!MVG
     ...
     call gw_drag_prof(ncol, band_mid, p, src_level, tend_level,  dt, &
        ...
        call effective_gw_diffusivity(ncol, band_mid, wavelength_mid, p, dt,     &
             ...
             gw_t_prime, k_wave_new, k_h_new, k_dyn_c, k_dyn_h, &
 	...
 	do k = 1, pver !add up contributions from all GWs sources
      ...
      var_gwt_tot(:,k) = var_gwt_tot(:,k) + var_gwt(:,k) ! MMK
      gw_t_prime_tot(:,k) = gw_t_prime_tot(:,k) + gw_t_prime(:,k)
...
if (use_gw_convect_sh) then
   ...
   IF (use_gw_chem) then  							!MVG
      ...
      call effective_gw_diffusivity(ncol, band_mid, wavelength_mid, p, dt,     &
            t, rhoi, nm, ni, c, tau, egwdffi, ubi, q, dse, dttke, tend_level,   &
            vramp, k_wave, & 
            gw_t_prime, k_wave_new, k_h_new, k_dyn_c, k_dyn_h, & ! MMK
            kappa_tilde, k_m_m, k_h_m, k_h_m_waccm, & 
             kvtt, gw_prndl, lchnk, & ! MMK END 
            k_e, zm, zi, var_gwt, k_dyn,		         &
      dttdf, ttgw, qtgw, state1%lat(:ncol), state1%lon(:ncol))
      ...
      do k = 1, pver !add up contributions from all GWs sources
         ...
         gw_t_prime_tot(:,k) = gw_t_prime_tot(:,k) + gw_t_prime(:,k) ! MMK
         k_wave_new_tot(:,k) = k_wave_new_tot(:,k) + k_wave_new(:,k) ! MMK
         k_h_new_tot(:,k) = k_h_new_tot(:,k) + k_h_new(:,k) ! MMK
      enddo
  ...
  if (use_gw_rdg_beta) then
     ...
     call outfld('TEGW', t ,  ncol, lchnk)
      ...
      IF (use_gw_chem) then                              !MVG
         ...
         k_wave_new, k_h_new, k_dyn_c, k_dyn_h, & ! MMK
         gw_t_prime, kappa_tilde, k_m_m, k_h_m, k_h_m_waccm, gw_prndl, & ! MMK
         k_e, k_dyn, egwdffi, &
  ...
  if (use_gw_chem) then 			!MVG
     ...
     ! MMK tots outfld
     call outfld ('gw_t_prime_tot', gw_t_prime_tot, ncol, lchnk)
...
subroutine gw_rdg_calc( &
   ...
   ptend, flx_heat, &
   k_wave_orog_tot, & 
   gw_t_prime_orog_tot, k_wave_new_orog_tot, k_h_new_orog_tot, k_dyn_c_orog_tot, k_dyn_h_orog_tot, &! MMK START
  ...
  real(r8), intent(out), optional :: gw_t_prime_orog_tot(ncol,pver)
  ...
  real(r8) :: gw_t_prime_orog(ncol,pver)
  ...
  real(r8) :: k_dyn_h_orog_local(ncol,pver+1) ! MMK new total thermal diffusivity
   ...
   do nn = 1, n_rdg
	...
	! Solve for the drag profile 
      ...
      ! MMK
      k_wave_new_orog_tot(:,k) = k_wave_new_orog_tot(:,k) + k_wave_new_orog_local(:,k)
      k_h_new_orog_tot(:,k) = k_h_new_orog_tot(:,k) + k_h_new_orog_local(:,k)
...
! MMK
   ...
   call addfld (trim(prefix)//'_gw_t_prime',(/ 'lev' /), 'A','m2/s', &
      trim(scheme)//' MMK effective wave diffusivity')
